  ...
  const deleteShipper = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:5001/shippers/${id}`)
              ...
              <Grid item key={key} xs={12} sm={6} md={4}>
                <Card className={classes.card}>
                  ...
                  <CardContent className={classes.cardContent}>
                    ...
                    <Typography>{shipper.shipperId}</Typography>
                    <Typography>{shipper.mobile}</Typography>
                  </CardContent>
                  <CardActions>
                    <Link
                      to={`/update/${shipper._id}`}
                      className="btn btn-primary"
                    >
                      Update
                    </Link>
                    <Button
                      onClick={() => deleteShipper(shipper._id)}
                      size="small"
                      color="secondary"
                    >
                      delete
